...
def test_sanitization_and_build_name():
    cfg = SimpleNamespace()
    # include weird chars in type/variant
    cfg.model = SimpleNamespace(type="My/Model<>", variant="v:1")
    cfg.workflow = SimpleNamespace(mode="end-to-end")
...
def test_cli_resume_auto_selects_latest_checkpoint(monkeypatch, tmp_path: Path):
