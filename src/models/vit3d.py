...
class VisionTransformer3D(nn.Module):
    """
    A self-contained 3D Vision Transformer for CT volume classification.
    This implementation does not rely on MONAI and uses efficient checkpointing.
    """
    def __init__(self, volume_size=(96, 224, 224), patch_size=(16, 16, 16), 
                 in_channels=1, num_classes=18, embed_dim=768, depth=12,
                 num_heads=12, mlp_ratio=4., qkv_bias=True, drop_rate=0.,
                 attn_drop_rate=0., use_checkpointing: bool = False, **kwargs: Any):
        super().__init__()
    ...
    def forward(self, x):
        ...
        # Prepend the CLS token and add positional embeddings.
        B = x.shape[0]
        ...
        # Apply final normalization and extract the CLS token for classification.
        x = self.norm(x)
        cls_token_final = x[:, 0]
...
def vit_tiny_3d(num_classes: int = 18, use_checkpointing: bool = False, **kwargs: Any) -> VisionTransformer3D:
...
def vit_small_3d(num_classes: int = 18, use_checkpointing: bool = False, **kwargs: Any) -> VisionTransformer3D:
...
def vit_base_3d(num_classes: int = 18, use_checkpointing: bool = False, **kwargs: Any) -> VisionTransformer3D:
...
def vit_large_3d(num_classes: int = 18, use_checkpointing: bool = False, **kwargs: Any) -> VisionTransformer3D:
