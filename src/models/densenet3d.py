...
class DenseNet3D(nn.Module):
    ...
    appending a custom classification head.
    """
    def __init__(self, growth_rate: int, block_config: Tuple[int, ...], 
                 num_init_features: int, num_classes: int, **kwargs):
        super().__init__()
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        # Extract features using the MONAI backbone.
        features = self.monai_densenet_base.features(x)
...
def densenet121_3d(num_classes: int = 18, **kwargs: Any) -> DenseNet3D:
    """Constructs a 3D DenseNet-121 model."""
    return DenseNet3D(growth_rate=32, block_config=(6, 12, 24, 16),
                      num_init_features=64, num_classes=num_classes, **kwargs)

def densenet169_3d(num_classes: int = 18, **kwargs: Any) -> DenseNet3D:
    """Constructs a 3D DenseNet-169 model."""
    return DenseNet3D(growth_rate=32, block_config=(6, 12, 32, 32),
                      num_init_features=64, num_classes=num_classes, **kwargs)

def densenet201_3d(num_classes: int = 18, **kwargs: Any) -> DenseNet3D:
    """Constructs a 3D DenseNet-201 model."""
    return DenseNet3D(growth_rate=32, block_config=(6, 12, 48, 32),
                      num_init_features=64, num_classes=num_classes, **kwargs)

def densenet161_3d(num_classes: int = 18, **kwargs: Any) -> DenseNet3D:
...
def densenet_small_3d(num_classes: int = 18, **kwargs: Any) -> DenseNet3D:
    """Constructs a small 3D DenseNet model."""
    return DenseNet3D(growth_rate=16, block_config=(4, 8, 12, 8),
                      num_init_features=32, num_classes=num_classes, **kwargs)

def densenet_tiny_3d(num_classes: int = 18, **kwargs: Any) -> DenseNet3D:
